...
model_data["normalized"] = True  # False
model_data["dir"] = './data/'
model_data["path"] = (model_data["dir"] + 'darcy2d_s' + str(model_data["grid_point_num"]) +
...
results = lbfgs_minimize(loss_func, initial_position=initial_pos, parallel_iterations=1,
                         max_iterations=100, num_correction_pairs=100, tolerance=1e-20)
# max_iterations = 1000, num_correction_pairs = 100, tolerance = 1e-20)
t3 = time.time()
...
model_evaluation(model, model_data, model_params, loss_fn, test_loader)
