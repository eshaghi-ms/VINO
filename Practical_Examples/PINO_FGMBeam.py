"""
Physics-Informed Fourier Neural Operator for a 2D elasticity problem for an FGM beam with
...
from utils.fno_utils import count_params, PinoFGMLoss, train_fno, model_evaluation, collate_fn
...
class FNO2d_FGM(FNO2D):
    ...
    def __call__(self, x_: jnp.ndarray) -> jnp.ndarray:
        ...
        # m = grid[..., 1:2] * (grid[..., 1:2] - self.model_data['length'])
        # x *= m
        return x_
...
model_data["beam"]["numPtsU"] = 64  # 32
model_data["beam"]["numPtsV"] = 32  # 16
...
model_data["learning_rate"] = 0.005  # 0.001
model_data["num_epoch"] = 1500  # 100
model_data["n_epochs_LBFGS"] = 500  # 100
model_data["nrg"] = random.PRNGKey(0)
model_data["normalized"] = True
model_data["data_type"] = 'float32'
model_data["dir"] = './data/'
model_data["path"] = (model_data["dir"] +
#                   'FGMBeam_LxW_' + str(beam_length) + "x" + str(beam_width) +
...
loss_fn = PinoFGMLoss(model, model_data, normalizers, d=1, p=1, size_average=False)
...
plot_field_1d(x, traction, "TestTraction", folder=folder)
plot_field_1d(y, material, "TestStiffnessDistribution", folder=folder)
