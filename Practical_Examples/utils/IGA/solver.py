...
from .materials import MaterialElast2D_FGM
from .IGA import IGAMesh2D
from .multipatch import gen_vertex2patch2D, gen_edge_list, zip_conforming
from .assembly import gen_gauss_pts, stiff_elast_FGM_2D
from .boundary import boundary2D, applyBCElast2D
from .postprocessing import plot_fields_2D, comp_measurement_values, get_measurements_vector, get_measurement_stresses
...
def elastic_beam(traction, elasticity_modulus, model_data):
    ...
    material = MaterialElast2D_FGM(Emod=Emod, nu=nu, vertices=vertices, gp=elasticity_modulus, N=numPtsV,
                                   plane_type="stress")
    ...
    num_pts_xi = numPtsU  # 2**(num_refinements+4)+2
    num_output_fields = 4
    meas_stress_all, meas_pts_phys_xy_all, vals_stress_min, vals_stress_max = (
        comp_measurement_values(num_pts_xi, num_pts_eta, mesh_list, sol0, get_measurement_stresses,
                                num_output_fields, material))
    elapsed = time.time() - t
