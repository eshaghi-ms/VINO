...
def bezier_extraction(knot, deg):
    ...
    a = deg + 1
    b = a + 1
    # Initialize C with the number of non-zero knot spans in the 3rd dimension
...
def bernstein_basis_derivative(u_hat, deg):
    """
    Algorithm A1.3 in Piegl & Tiller
    xi is a 1D array
    ...
    dB0 = np.transpose(np.array([np.zeros(len(u_hat))]))
...
def form_extended_knot(localKnot, deg):
    ...
    Parameters
    ----------
    localKnot: the local knot vector (list of length deg+2)
...
def bernstein_basis_2d(pts_u, pts_v, deg):
    ...
    dB_u = bernstein_basis_derivative(pts_u, deg[0])
    dB_v = bernstein_basis_derivative(pts_v, deg[1])
