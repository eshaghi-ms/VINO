...
def gen_vertex2patch2D(patch_list):
    ...
    patch2vertex : (list of arrays of length 4) list which gives the patch to 
                    vertex connectivity, where the vertices are in counter-clockwise
                    order starting from the origin of the parameter space (u=0, v=0)
...
def gen_edge_list(patch2vertex):
    ...
    edge_list : dict where each entry contains either an array
                ...
                direction_flag = 1 if the parametric direction matches along the
    ...
    for i in range(num_patches):
        ...
        for j in range(numVerts):
            ...
            if edge_key in edge_list:
                ...
                # order or both in decreasing order, then the parametric
...
def sort_edge_nodes_element(mesh, edge, flag_dir):
    ...
    nodes : (list) control point indices in the order that they appear along
              the patch interface
    elements : (list) element indices in the order that they appear along the
                patch interface
    """
...
def zip_conforming(mesh_list, vertex2patch, edge_list, output_nodes=False):
    """
    Connects conforming patches by creating the elem_nodes_global fields
    from the vertex2patch and edge_list connectives
    ...
    for vert_index in range(len(vertex2patch)):
        for i in range(len(vertex2patch[vert_index])):
            ...
            if assigned_nodes[patch_index] is None:
    ...
    if output_nodes is True:
        ...
        for i, mesh in enumerate(mesh_list):
            IEN.append(np.array(mesh.elem_node_global))
            nodes.append(mesh.nodes)
        return size_basis, nodes, IEN
