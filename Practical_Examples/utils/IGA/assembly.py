...
def stiff_2D(mesh_list, a0, gauss_rule):
    """
    Assembles the stiffness matrix K_ij = ∫ a0(x,y)∇ϕ_i(x,y)∇ϕ_j(x,y) dΩ
        ...
        for i_elem in range(mesh_list[i_patch].num_elem):
            ...
            for j_gauss in range(num_gauss_v):
                for i_gauss in range(num_gauss_u):
                    ...
                    local_area = jac_par_phys * jac_ref_par * wgts_u[i_gauss] * wgts_v[j_gauss]
                    local_stiff += local_area * a0(phys_pt[0], phys_pt[1]) * (dR.transpose() @ dR)
        ...
        for i_elem in range(mesh_list[i_patch].num_elem):
            ...
            for j_gauss in range(num_gauss_v):
                for i_gauss in range(num_gauss_u):
                    ...
                    local_area = jac_par_phys * jac_ref_par * wgts_u[i_gauss] * wgts_v[j_gauss]
                    local_mass += local_area * a1(phys_pt[0], phys_pt[1]) * np.outer(RR, RR)
                    domain_area += local_area
...
def stiff_elast_2D(mesh_list, Cmat, gauss_rule):
    """
    Assembles the stiffness matrix Kₑ = ∫ B^T*C*B dΩ
...
def stiff_elast_FGM_2D(mesh_list, material, gauss_rule):
    ...
    ----------
    mesh_list : (list of IGAMesh2D) multi patch mesh
    material : (2D array) elasticity matrix
    gauss_rule : list of Gauss points and weights in the reference interval [-1,1]
                 (one for each direction)
