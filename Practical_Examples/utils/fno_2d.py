...
class SpectralConv2d(nn.Module):
    ...
    def __call__(self, x):
        ...
        # Given an input signal of dimensions (N, C, H, W), the
        # output signal will have dimensions (N, C, H, W//2+1).
        # Therefore, the kernel weights will have different dimensions
        # for the two axis.
...
class FourierStage(nn.Module):
    ...
    def __call__(self, x):
        ...
        x_fourier = nn.Conv(self.out_channels, (1, 1))(x_fourier)
        x_fourier = nn.gelu(x_fourier)
        x_fourier = nn.Conv(self.out_channels, (1, 1))(x_fourier)
        x_local = nn.Conv(self.out_channels, (1, 1))(x)
  ...
  """
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        for depthnum in range(self.depth):
            activation = self.activation if depthnum < self.depth - 1 else lambda _x: _x
            x = FourierStage(
