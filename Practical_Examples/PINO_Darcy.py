...
from utils.fno_utils import count_params, PinoDarcyLoss, train_fno, model_evaluation, collate_fn


class FNO2d_darcy(FNO2D):
    def __call__(self, _x):
        _x = super().__call__(_x)
        # apply Neumann boundary condition
        _x = _x.at[:, :, 0, :].set(0)     # rel error (150 it) = 3.7 %
        _x = _x.at[:, :, -1, :].set(0)
        _x = _x.at[:, 0, :, :].set(0)
        _x = _x.at[:, -1, :, :].set(0)

        # grid = self.get_grid(x)     # rel error (150 it) = 5 %
...
model_data["learning_rate"] = 0.001  # 0.0001
model_data["weight_decay"] = 1e-4
model_data["n_epochs"] = 150 #500
model_data["n_epochs_LBFGS"] = 150 #500
...
loss_fn = PinoDarcyLoss(model, model_data, normalizers, d=1, p=1, size_average=False)
